...
public class ControllerObserver implements ControllerInterface<DTOResponseObserver, DTORequestObserver> {
    ...
    public ResponseEntity<Page<DTOResponseObserver>> retrieve(@RequestParam(name = "key", defaultValue = "", required = false) String key, @RequestParam(name="value", defaultValue = "", required = false) String value, Pageable pageable){
        return ResponseEntity.ok().body(serviceObserver.retrieve(pageable, key, value));
    }
