...
public class ServiceCommission implements ServiceInterface<DTOResponseCommission, DTORequestCommission> {
    ...
    public Page<DTOResponseCommission> retrieve(Pageable pageable, String filter){
        switch (pageable.getSort().toString().substring(0, pageable.getSort().toString().length() - 5)) {
            case "id": {
                return repositoryCommissionPage.findByIdOrderByIdAsc(pageable, UUID.fromString(filter)).map(MapStruct.MAPPER::toDTO);
            }
            default: {
