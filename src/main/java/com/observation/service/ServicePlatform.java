...
public class ServicePlatform implements ServiceInterface<DTOResponsePlatform, DTORequestPlatform> {
    ...
    public Page<DTOResponsePlatform> retrieve(Pageable pageable, String key, String value){
        switch (key) {
            case "id": {
                return repositoryPlatform.findByIdOrderByIdAsc(pageable, UUID.fromString(value)).map(MapStruct.MAPPER::toDTO);
            }
            case "name": {
                return repositoryPlatform.findByNameContainingIgnoreCaseOrderByNameAsc(pageable, value).map(MapStruct.MAPPER::toDTO);
            }
            default: {
                return repositoryPlatform.findAll(pageable).map(MapStruct.MAPPER::toDTO);
            }
        }
    }
